...
public class RiftParticlesTickPacket {
    ...
    public RiftParticlesTickPacket(FriendlyByteBuf buf) {
    ...
    public RiftParticlesTickPacket(BlockPos pos, int radius, int particleCount, int lineCount) {
        this.blockPos = pos;
        this.radius = radius;
    ...
    public static void encode(RiftParticlesTickPacket msg, FriendlyByteBuf buf) {
    ...
    public static RiftParticlesTickPacket decode(FriendlyByteBuf buf) {
        return new RiftParticlesTickPacket(buf.readBlockPos(), buf.readInt(), buf.readInt(), buf.readInt());
    }

    public static void handle(RiftParticlesTickPacket msg, NetworkEvent.Context ctx) {
        ctx.enqueueWork(() -> {
    ...
    @OnlyIn(Dist.CLIENT)
    private static void handleOnClient(RiftParticlesTickPacket msg) {
    ...
    public static void spawnParticles(ClientLevel level, BlockPos riftPos, int radius, int particleCount, int lineCount) {
        ...
        for (int line = 0; line < lineCount; line++) {
            ...
            for (int i = 0; i < particleCount / lineCount; i++) {
                ...
                double speed = 0.05 + (distance / radius) * 0.1; // Particles further away move faster
