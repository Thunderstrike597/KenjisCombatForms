...
public class RiftSummonParticlesTickPacket {
    ...
    public RiftSummonParticlesTickPacket(FriendlyByteBuf buf) {
    ...
    public RiftSummonParticlesTickPacket(BlockPos pos, int radius, int particleCount, int lineCount) {
        this.blockPos = pos;
        this.radius = radius;
    ...
    public static void encode(RiftSummonParticlesTickPacket msg, FriendlyByteBuf buf) {
    ...
    public static RiftSummonParticlesTickPacket decode(FriendlyByteBuf buf) {
        return new RiftSummonParticlesTickPacket(buf.readBlockPos(), buf.readInt(), buf.readInt(), buf.readInt());
    }

    public static void handle(RiftSummonParticlesTickPacket msg, NetworkEvent.Context ctx) {
        ctx.enqueueWork(() -> {
    ...
    @OnlyIn(Dist.CLIENT)
    private static void handleOnClient(RiftSummonParticlesTickPacket msg) {
    ...
    public static void spawnParticles(ClientLevel level, BlockPos riftPos, int radius, int particleCount, int lineCount) {
        ...
        for (int line = 0; line < lineCount; line++) {
            ...
            for (int i = 0; i < particleCount / lineCount; i++) {
                ...
                // Spawn the particle
                level.addParticle(
                        ModParticles.VOID_RIFT_PARTICLE.get(),
                        x, y, z,
